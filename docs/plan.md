Below is a practical, staged plan to evolve PAC into a dual‑library manager that maintains a "master" FLAC library and keeps a lossy mirror (Opus by default, AAC optional). It builds on the existing architecture (scanner → index → planner → scheduler) and adds a "FLAC Library" pipeline with strong verification, compression policy, artwork extraction, and parallel controls. It also includes how to track runs and issues.
...
- pac.library_planner
  - Produces actions for FLAC maintenance:
    - test_integrity, resample_to_cd, recompress, extract_art, skip, hold.
  - Inputs: scan of FLAC sources, config, DB, tool availability.
  - Rules:
    - Early‑stop: On first "issue" (error per config) → emit hold and do not queue further actions for that file.
...
- Keep using observations for append‑only history: events like flac_test_ok, flac_test_err, recompress_ok, recompress_skip, resample_ok, art_export_ok. This gives you a lightweight audit log.
...
- New top‑level command: pac library
  - Inputs
    ...
    - --stop-on "error|never" (default error)
    - --dry-run: plan only
...
- Phase 0: Preflight
  - Detect flac, metaflac, ffmpeg/sox availability; record versions.
...
- Phase 2: Resample to CD if needed
...
- JSON lines log (existing): add action values: flac_test, flac_recompress, flac_resample, art_export with status=ok|skip|warn|error|suspect and structured fields.
...
CLI examples
- Dry run:
  - pac library --root ~/Music/FLAC --target-compression 8 --dry-run
- Run and mirror:
...
Testing strategy
- Unit tests
  - planner decisions: downsample needed, recompress skip on COMPRESSION tag, early-stop on test error.
...
- Issues per task with clear DoD; link PRs; labels: area:flac, area:art, perf, db.
...
- Early stop: treat "suspect" as configurable stop; never recompress or mirror such files in the same run.
...
If you want, I can draft the exact function signatures (and stub implementations) for pac.flac_tools, pac.library_planner, and pac.library_runner next, and a CLI wiring for pac library that mirrors how convert-dir is implemented.